...
public class PasteBinMetaDataTask extends AbstractNetJSTask {
	...
	public void run() {
		...
		try {
			...
			result.put("visits_count", visits != null ? Integer.parseInt(visits.text().replaceAll(",| ", "")) : null);
			Element stars = elems.getElementsByClass("rating js-post-rating").first();
			result.put("stars_count", stars != null ? Integer.parseInt(stars.text().replaceAll(",| ", "")) : null);
